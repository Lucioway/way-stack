...
### Why
The agent factory previously shipped only the condensed SDD flow, and the stack had no deployment story. This release ports the complete knowledge base behind the factory — full framework, design principles, tooling stack, fleet monitoring — plus a security-first deploy agent, so projects generated by the stack can also ship safely without external dependencies.
